...
public class TileMap {
  ...
  private HashMap<String, map.Coordinate> leaderCoordinates; // a map that holds the values of the start and the end of the map
  ...
  public void generateMap() {
    ...
    HashMap<map.Coordinate, TileType> leaderMap = new HashMap<map.Coordinate, TileType>();
    ...
    for (int i = 0; i < numberOfLaketileLeaders; i++) {
      ...
      map.Coordinate tmp = new map.Coordinate(randomX, randomY);
    ...
    for (int i = 0; i < mapSize; i++) {
      for (int j = 0; j < mapSize; j++) {
        ...
        for (java.util.Map.Entry<map.Coordinate, TileType> entry : leaderMap.entrySet()) {
  ...
  private HashMap<String, map.Coordinate> generateRoads() {
    int roadStartX = 0;
    int roadStartY = 0;
    ...
    HashMap<Integer, map.Coordinate> leaders = new HashMap<Integer, map.Coordinate>();
    ...
    leaders.put(1, new map.Coordinate(roadStartX, roadStartY));
    
    // second point
    leaders.put(2, new map.Coordinate(randomNumberInGivenRange(mapSize / 8 + 1, mapSize / 4),
                                  randomNumberInGivenRange(2, mapSize - 2)));
    
    // third point
    leaders.put(2, new map.Coordinate(randomNumberInGivenRange(mapSize / 4 + 1, mapSize / 2),
                                  randomNumberInGivenRange(2, mapSize - 2)));
    
    // final point
    roadEndX = randomNumberInGivenRange(2, mapSize - 1);
    roadEndY = randomNumberInGivenRange(2, mapSize - 2);
    leaders.put(1, new map.Coordinate(roadEndX, roadEndY));
    ...
    HashMap<String, map.Coordinate> tmp = new HashMap<String, map.Coordinate>();
    tmp.put("Start", new map.Coordinate(roadStartX, roadStartY));
    tmp.put("End", new map.Coordinate(roadEndX, roadEndY));
